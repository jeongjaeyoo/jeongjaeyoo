...
character_x_pos = (screen_width / 2) - (character_width/2) # 화면 가로의 절반 크기에 해당하는 곳에 캐릭터를 위치시킴 (가로 위치)
...
enemy_x_pos = (screen_width / 2) - (character_width/2) # 화면 가로의 절반 크기에 해당하는 곳에 캐릭터를 위치시킴 (가로 위치)
...
while running:
    ...
    for event in pygame.event.get():  # 어떤 이벤트가 발생하였는가?
        ...
        if event.type == pygame.KEYUP: #방향끼를 뗴면 멈춤 
